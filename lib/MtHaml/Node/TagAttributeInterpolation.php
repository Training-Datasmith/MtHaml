...
class Tag_Attribute_Interpolation extends Tag_Attribute
{
    ...
    public function accept(Node_Visitor_Interface $visitor)
    {
        if (false !== $visitor->enter_tag_attribute($this)) {
            if (false !== $visitor->enter_tag_attribute_interpolation($this)) {
                $this->get_value()->accept($visitor);
            }
            $visitor->leave_tag_attribute_interpolation($this);
        }
